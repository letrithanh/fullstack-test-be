...
describe("Event Routes", () => {
    ...
    describe("POST /events/:id/register", () => {
        it("should return 201 Created and the created registration if the data is valid", async () => {
            // Create a test event
            const createdEvent = await PRISMA.event.create({
                data: validEventData,
            });

            const validAttendeeData = {
                name: "Valid Name",
                gender: "MALE",
        ...
        it("should return 500 Internal Server Error if the request body is invalid", async () => {
            ...
            const invalidAttendeeData = {
                name: "",
                gender: "MALE",
                email: "invalidAttendee@example.com",
                phone: "0123456789",
            };
        ...
        it("should return 500 Internal Server Error if the event is full", async () => {
            // Create a test event
            const createdEvent = await PRISMA.event.create({
                data: { ...validEventData, maxAttendees: 1 },
            });

            const validAttendeeData1 = {
                name: "Valid Name",
                gender: "MALE",
                email: "email@example.com",
                phone: "0123456789",
            ...
            await request(app)
                .post(`/events/${createdEvent.id}/register`)
                .send(validAttendeeData1)
                .set("Content-Type", "application/json")
                .set("Accept", "application/json");

            // Try to register a second attendee
            const response = await request(app)
                .post(`/events/${createdEvent.id}/register`)
                .send(validAttendeeData2)
                .set("Content-Type", "application/json")
                .set("Accept", "application/json");

            expect(response.status).toBe(500);
            expect(response.body).toEqual({ message: "Event is full" });
